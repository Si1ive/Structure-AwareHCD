...
class SpectrumMambaBlock(nn.Module):
    def __init__(
            self,
            hidden_dim: int = 0,
            drop_path: float = 0,
        ...
        self.self_attention = SpectrumSSM(d_model=hidden_dim, dropout=attn_drop_rate, d_state=d_state,
                                                dt_init=dt_init, **kwargs)
        self.drop_path = DropPath(drop_path)

        self.cpe2 = nn.Conv2d(hidden_dim, hidden_dim, 3, padding=1, groups=hidden_dim)
        self.ln_2 = norm_layer(hidden_dim)
        self.mlp = MLP(in_features=hidden_dim, hidden_features=int(hidden_dim * mlp_ratio), act_layer=mlp_act_layer,
                       drop=mlp_drop_rate, channels_first=False)

    def forward(self, x: torch.Tensor):
        # x = x + self.cpe1(x.permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1)
        x = x + self.cpe1(x.contiguous())
        x = x + self.drop_path(self.self_attention(self.ln_1(x.permute(0, 3, 2, 1).contiguous()))).permute(0, 3, 2, 1)
        x = x + self.cpe2(x.contiguous())
        x = x + self.drop_path(self.mlp(self.ln_2(x.permute(0, 3, 2, 1).contiguous()))).permute(0, 3, 2, 1)
        return x
