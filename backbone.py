...
class Backbone(nn.Module):
    def __init__(self, backbone, patch_size, band_size, dim):
        ...
        # 孪生网络
        if backbone == 'resnet':
            self.layer1 = BasicBlock( 64,64, 1, stride = 1)
            self.layer2 = BasicBlock( 64, 64, stride = 1)
            self.layer3 = BasicBlock( 64, 128, stride = 1)
            self.layer4 = BasicBlock( 128, 128, stride = 1)
        elif backbone == 'mamba':
            self.layer1 = SpatialMambaBlock(hidden_dim=64,drop_path=0.,attn_drop_rate=0.)
            self.layer2 = SpatialMambaBlock(hidden_dim=64,drop_path=0.,attn_drop_rate=0.)
            self.backbone_up     = BasicBlock(64, 128, stride=1)
            self.layer3 = SpatialMambaBlock(hidden_dim=128,drop_path=0.,attn_drop_rate=0.)
            self.layer4 = SpatialMambaBlock(hidden_dim=128,drop_path=0.,attn_drop_rate=0.)
        # fuse网络的Resblock

        self.layer01 = BasicBlock( 64,64,  stride=1)
        self.layer02 = BasicBlock( 64,64,  stride=1)
        self.layer03 = BasicBlock( 64,128,  stride=1)
        self.layer04 = BasicBlock( 128,128, stride=1)
        # cross attention
    ...
    def forward(self, x1, x2):
        ...
        out1 = self.backbone_up(out1)
        out2 = self.backbone_up(out2)
